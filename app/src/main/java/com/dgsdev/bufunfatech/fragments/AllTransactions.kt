...
import com.dgsdev.bufunfatech.databinding.FragmentAllTransactionsBinding
...
class AllTransactions : Fragment() ,View.OnClickListener {
    ...
    private fun showMonthlyTransactions() {
        binding.text.text = "Transações Mensais"
        ...
        binding.title.text = "Transações Mensais"
    ...
    private fun showPiChart() {
        ...
        mPieChart.addPieSlice(PieModel("Saúde", totalSaude, ContextCompat.getColor(requireContext(), R.color.red)))
        ...
        mPieChart.addPieSlice(PieModel("Educação", totalEducacao, ContextCompat.getColor(requireContext(), R.color.green)))
    ...
    private fun showYearlyTransactions() {
        binding.title.text = "Transações Anuais"
        binding.text.text = "Orçamento Anual"
        year=SimpleDateFormat("yyyy").format(Calendar.getInstance().getTime()).toInt()
        val list = mutableListOf(2020)
        list.clear()
        for(i in year downTo 2020){
