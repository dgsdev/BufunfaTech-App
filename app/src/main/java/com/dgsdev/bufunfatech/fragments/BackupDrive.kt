...
import com.dgsdev.bufunfatech.databinding.FragmentBackupDriveBinding
...
class BackupDrive : Fragment() {
    ...
    private fun setData() {
        ...
        binding.backupBtn.setOnClickListener{
            //backupdata()
            Toast.makeText(requireActivity(),"Em breve, aguarde uma atualização.", Toast.LENGTH_SHORT).show()
    ...
    private fun Download(googleDriveService:Drive) {
        try {
            val dir = java.io.File("/data/data/com.dgsdev.bufunfatech/databases")
            if (dir.isDirectory) {
